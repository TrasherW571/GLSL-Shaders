...
void Camera::mouseClicked(float x, float y, bool shift, bool ctrl, bool alt)
{
	mousePrev.x = x;
	mousePrev.y = y;
	if(shift) {					// If shift key is being pressed we set object state to that it will be translated
		state = Camera::TRANSLATE;
	} else if(ctrl) {			// If ctrl key is being pressed we set object state to that it will scale it
		state = Camera::SCALE;
	} else {
		state = Camera::ROTATE;
	}
